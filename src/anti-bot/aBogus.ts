/**
 * a_bogus版本实现；Desktop 1.0.1.7核心使用显式上下文，其他参考版本不可互换。
 */
...
export const BDMS_PRESETS = {
  ...
  /** 旧参考配置沿用同组常量；不构成 Desktop 版本对齐证据。 */
...
export const MAC_SCREEN_FINGERPRINT =
  '1728|1117|1728|303|1728|1117|1728|982|MacIntel';

/** 参考脚本默认 Windows 指纹 */
export const WIN_SCREEN_FINGERPRINT =
  '1920|366|1918|1048|1920|1050|1920|1080|Win32';
...
/** State supplied by the same BDMS execution context that owns this request. */
export interface DesktopABogusOptions {
  /** URLSearchParams-serialized query after msToken, before a_bogus. */
  query: string;
  /** Normalized string body; GET uses ''. No form serialization is done here. */
  body: string;
  /** Wrapper-normalized UA; the core applies trim() and UTF-16, not UTF-8 RC4. */
  userAgent: string;
  aid: number;
  pageId: number;
  /** M(), not a fixed platform/version value. */
  environmentMask: number;
  /** q(), or zero when the originating track mode is nonzero. */
  behaviorMask: number;
  /** onwheelx._Ax descriptor state: 11 absent, 3 non-writable, 12 otherwise. */
  flag: 3 | 11 | 12;
  /** Source-ordered 17-field window fingerprint, including platform. */
  fingerprint: string;
  /** Core Date.now() sample. */
  nowMs: number;
  /** navigator.vendorSubs.ink captured by the wrapper; not a random time offset. */
  inkMs: number;
  /** Defaults to Math.random as in BDMS; exactly three values in [0,1). */
  random?: () => number;
}

/**
 * Desktop BDMS 1.0.1.7 core, verified against unchanged source XHR output.
 * Deliberately requires environment state: this does not collect a browser
 * fingerprint, issue msToken, initialize BDMS, or prove server acceptance.
 */
export function generateDesktopABogus(options: DesktopABogusOptions): string {
  for (const value of [options.aid, options.pageId, options.environmentMask, options.behaviorMask, options.nowMs, options.inkMs]) {
    if (!Number.isSafeInteger(value)) throw new RangeError('BDMS requires explicit integer state');
  }
  if (![3, 11, 12].includes(options.flag)) throw new RangeError('Invalid BDMS descriptor flag');
  for (const value of [options.query, options.body, options.userAgent, options.fingerprint]) {
    if (typeof value !== 'string') throw new TypeError('BDMS requires explicit string inputs');
  }
  return generateDesktopABogusWithRuntime(options, {
    flag: () => options.flag, now: () => options.nowMs, ink: () => options.inkMs,
    pageId: () => options.pageId, aid: () => options.aid, fingerprint: () => options.fingerprint,
    random: () => (options.random ?? Math.random)(),
  });
}

/** @internal Live getters preserve core sampling order without duplicating the cryptographic core. */
export interface DesktopABogusRuntime {
  flag(): 3 | 11 | 12;
  now(): number;
...
/** @internal Shared implementation for snapshots and one-context request signing. */
export function generateDesktopABogusWithRuntime(
  options: Pick<DesktopABogusOptions, 'query' | 'body' | 'userAgent' | 'environmentMask' | 'behaviorMask'>,
  runtime: DesktopABogusRuntime,
): string {
  const m = options.environmentMask, b = options.behaviorMask;
  const flag = runtime.flag();
  const t = runtime.now();
  const queryHash = sm3Digest(sm3Digest(options.query + 'cus'));
  const bodyHash = sm3Digest(sm3Digest(options.body + 'cus'));
  const uaHash = sm3Digest(lmStrEncode(
    desktopRc4([byteAt(m, 1), byteAt(m, 0), byteAt(b, 0)], utf16Units(options.userAgent.trim())),
    TABLES.s3,
  ));
  const ink = runtime.ink() || 1000;
  const p = runtime.pageId(), a = runtime.aid();
  const fingerprint = runtime.fingerprint();
  const fp = utf16Units(fingerprint).flatMap(code => code > 255 ? [code >> 8, code & 255] : [code]);
  if (fp.length > 65535) throw new RangeError('BDMS fingerprint exceeds its 16-bit length field');
  const header = [
    44, byteAt(t, 3), byteAt(p, 3), 0, byteAt(m, 1), byteAt(b, 3), byteAt(a, 1),
    queryHash[21]!, bodyHash[21]!, byteAt(p, 2), uaHash[23]!, byteAt(t, 2), 0,
    byteAt(p, 1), byteAt(p, 0), byteAt(m, 0), byteAt(b, 2), byteAt(a, 0),
    queryHash[22]!, bodyHash[22]!, uaHash[24]!, byteAt(t, 1), 0, byteAt(m, 3),
    byteAt(a, 3), byteAt(b, 1), byteAt(t, 0), 0, byteAt(m, 2), byteAt(b, 0),
    byteAt(ink, 3), byteAt(ink, 2), byteAt(a, 2), byteAt(ink, 1), byteAt(ink, 0),
    flag, Math.trunc(ink / 2 ** 32) & 255, Math.trunc(ink / 2 ** 40) & 255,
    Math.trunc(t / 2 ** 32) & 255, Math.trunc(t / 2 ** 40) & 255,
    byteAt(fp.length, 0), byteAt(fp.length, 1), 0, 0,
  ];
  const checksum = header.reduce((value, item) => value ^ item, 0);
  const mix = (first: number, second: number): number[] => {
    const value = runtime.random();
    if (!(value >= 0 && value < 1)) throw new RangeError('BDMS random value must be in [0,1)');
    const word = Math.trunc(value * 10000);
    const low = word & 255;
    const high = (word >> 8) & 255;
    return [(low & 170) | (first & 85), (low & 85) | (first & 170),
      (high & 170) | (second & 85), (high & 85) | (second & 170)];
  };
  return lmStrEncode([
    ...mix(3, 45), ...mix(1, 0), ...mix(1, 7),
    ...desktopRc4([121], [...header, ...fp, checksum]),
  ], TABLES.s4);
}
...
/** BDMS .7 report strData, not a_bogus: one random byte, RC4, then s1 base64. */
export function encodeDesktopReportData(serializedReport: string | undefined, math: Pick<Math, 'floor' | 'random'> = Math): string {
  const key = String.fromCharCode(math.floor(math.random() * 256)).charCodeAt(0);
  // The source uses JS code units and base64 masks each to its low byte, not UTF-8.
  // An undefined JSON result throws here after drawing the key, as the source does.
  return lmStrEncode([65, key & 255, ...desktopRc4([key], utf16Units(serializedReport!)).map(value => value & 255)], TABLES.s1);
}

/** Standard RC4, unlike the modified KSA used by the other reference versions. */
function desktopRc4(key: number[], input: number[]): number[] {
  const state = Array.from({ length: 256 }, (_, index) => index);
  let previous = 0;
  for (let index = 0; index < 256; index++) {
    previous = (previous + state[index]! + key[index % key.length]!) & 255;
    [state[index], state[previous]] = [state[previous]!, state[index]!];
  }
  previous = 0;
  return input.map((value, offset) => {
    const index = (offset + 1) & 255;
    previous = (previous + state[index]!) & 255;
    [state[index], state[previous]] = [state[previous]!, state[index]!];
    return value ^ state[(state[index]! + state[previous]!) & 255]!;
  });
}
...
export function generateJumpbyteABogus(options: JumpbyteABogusOptions): string {
  ...
  const uaHash = sm3Digest(
    lmStrEncode(
      jumpbyteGarble(jumpbyteUaSbox(0), encodeUtf8ForSm3(options.userAgent)),
      TABLES.s3,
    ),
  );
...
function cryptoRandomFloat(): number {
  // The shared Desktop core must also load in a browser without Node built-ins.
  // Preserve the legacy 64-bit big-endian draw and upper 53-bit conversion.
  const bytes = globalThis.crypto.getRandomValues(new Uint8Array(8));
  const value = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getBigUint64(0) >> 11n;
