...
/** Pure AWS Signature V4 implementation used by ByteDance VOD upload. */
...
export class ImMediaUploader {
  ...
  async uploadFile(data: Uint8Array, name: string): Promise<import('./cards.js').FileAsset> {
    if (!name.trim() || data.length === 0 || data.length > 10 * 1024 * 1024) throw new Error('file requires a name and 1 byte to 10 MiB of data');
  ...
  private async credentials(configKey = 'public_image_config'): Promise<UploadCredentials> {
    const deviceId = this.client.getDeviceId?.() ?? await this.deviceId();
    ...
    const response = await this.fetcher(`${UPLOAD_CONFIG_URL}?${params}`, {
      headers: this.browserHeaders('https://www.douyin.com/'),
    });
  ...
  private async putObject(address: UploadAddress, data: Uint8Array): Promise<void> {
    const response = await this.fetcher(`https://${address.host}/upload/v1/${address.storeUri}`, {
      method: 'POST', headers: {
    ...
    if (!response.ok || Number(result['code']) !== 2000) throw new Error(`TOS upload failed: ${result['message'] ?? response.status}`);
  }

  private async initParts(address: UploadAddress): Promise<string> {
    const boundary = `----WebKitFormBoundary${randomId(16)}`;
    const response = await this.fetcher(`https://${address.host}/upload/v1/${address.storeUri}?phase=init`, {
      method: 'POST',
  ...
  private async transferPart(address: UploadAddress, uploadId: string, part: number, crc: string, data: Uint8Array): Promise<void> {
    ...
    const response = await this.fetcher(`https://${address.host}/upload/v1/${address.storeUri}?${query}`, {
      method: 'POST',
  ...
  private async finishParts(address: UploadAddress, uploadId: string, manifest: string): Promise<void> {
    ...
    const response = await this.fetcher(`https://${address.host}/upload/v1/${address.storeUri}?${query}`, {
      method: 'POST', headers: { ...(await this.storageHeaders(address.authorization)), 'Content-Type': 'text/plain;charset=UTF-8' }, body: manifest,
    });
    const result = asRecord(await response.json());
  ...
  private async signedVodJson(
    ...
    const response = await this.fetcher(`${VOD_URL}?${signed.canonicalQuery}`, {
      method, headers: { ...signed.headers, Authorization: signed.authorization, 'User-Agent': this.client.getUserAgent(), ...(body ? { 'Content-Type': 'text/plain;charset=UTF-8' } : {}) },
      ...(body ? { body } : {}),
    });
    const json = asRecord(await response.json());
  ...
  private browserHeaders(referer: string): Record<string, string> {
    return { 'User-Agent': this.client.getUserAgent(), Cookie: this.client.getCookies(), Referer: referer };
  }
  ...
  private async storageHeaders(authorization: string, crc?: string): Promise<Record<string, string>> {
    const headers: Record<string, string> = {
      ...
      'X-Storage-U': await this.userId(), 'User-Agent': this.client.getUserAgent(),
    };
